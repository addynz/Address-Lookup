...
// AddyComplete v2.1.0.7 - https://www.addy.co.nz
...
function AddyComplete(input, fields) {
    ...
    me.widget.addService("addy", function (query, callback) {
        if (query.length < 3) {
            ...
            me.widget._hideResults();
            return;
        }		
		
        makeRequest('api/search?key=' + me.key + '&session=' + sessionId + '&expostbox=' + me.options.excludePostBox + '&max=' + me.widget.getOption("max_results") + '&s=' + query, function (data) {
    ...
    if (me.key.toLowerCase() === "demo-api-key") {
